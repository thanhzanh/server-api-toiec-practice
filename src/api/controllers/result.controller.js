...
// [POST] /api/results/submit-exam-test
module.exports.submitExamTest = async (req, res) => {
    try {
        const {
            id_nguoi_dung,
            id_bai_thi,
        ...
        // Tìm đề thi đầu vào
        const deThi = await BaiThi.findByPk(id_bai_thi);
        if (!deThi || deThi.la_bai_thi_dau_vao !== true) {
            return res.status(400).json({ message: "Đề thi đầu vào không tồn tại." });
        }
